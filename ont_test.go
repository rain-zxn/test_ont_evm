...
//	return
//}
func (this *MakeTxParamWithSender) Serialization1() (data []byte, err error) {
	sink := ontcommon.NewZeroCopySink(nil)
...
func Compose(tx *msg.Tx) (err error) {
	...
	if len(v) == 0 {
		...
		fmt.Println("GetSideChainMsg:", msg)
	...
	fmt.Println("ontcommon.ToHexString(param.TxHash):", ontcommon.ToHexString(param.TxHash))
	...
	itemValue, err := makeTxParamWithSender.Serialization1()
	if err != nil {
	...
	fmt.Println("string(path):", ontcommon.ToHexString(path))
	tx.SrcProof = path
	{
	...
	proof := tx.SrcProof
	source := ontcommon.NewZeroCopySource(tx.SrcStateRoot)
	crossChainMsg := new(types.CrossChainMsg)
	err = crossChainMsg.Deserialization(source)
	if err != nil {
		fmt.Println("err Deserialization(source):", err)
	}
	v, err = merkle.MerkleProve(proof, crossChainMsg.StatesRoot)
	if err != nil {
		fmt.Println("err MerkleProve(proof, crossChainMsg.StatesRoot):", err)
	}
	fmt.Println("v:", ontcommon.ToHexString(v))
	return nil
}
...
func Test_ontCrossT1(t *testing.T) {
	s := "34d4a23A1FC0C694f0D74DDAf9D8d564cfE2D430"
	fmt.Println(HexStringReverse1(s))
}
